...
public class UserChat extends UserChatDesigner {
    ...
    SwingUtilities.invokeLater(() -> {
      ...
      btnLock.addActionListener(e -> {
        if (btnLock.getText().equalsIgnoreCase("✔")) {
          ...
          lblTabTitle.setFont(
              lblTabTitle.getFont().deriveFont(lblTabTitle.getFont().getStyle() | Font.BOLD));
        } else {
          btnLock.setText("✔");
  ...
  private void registerListener() {
    ...
    btnUploadHastebin.addActionListener(e -> {
      ...
      appendComment("Hastebin-URL: " + HastebinAPI.upload(result.toString(), true, false)
  ...
  public void open() {
    ...
    appendComment(
        "\uD83D\uDC4B Chat geöffnet am " + new SimpleDateFormat("dd.MM.yyyy 'um' HH:mm:ss.SS")
            .format(new Date()));
  }
  ...
  public void close(boolean force) {
    ...
    appendComment(
        "\uD83D\uDD12 Chat geschlossen am " + new SimpleDateFormat("dd.MM.yyyy 'um' HH:mm:ss.SS")
            .format(new Date()));
  ...
  /**
   * Aktualisiert den Tab-Title Prüft nach ungelesenen Nachrichten
   */
  ...
  public void received(String message) {
    ...
    if (!inFocus()) {
      ...
      if (Settings.msgPlaySoundOnMessage) {
        SoundEvent event = new SoundEvent(new ResourceLocation(Settings.msgSoundFileName));
  ...
  /**
   * Sendet die Nachricht, die in das Textfeld eingegeben wurde. Anschließend wird die
   * Eingabetextbox ausgewählt
   */
  private void send() {
    ...
    // Nachricht senden
    if (Minecraft.getInstance().player != null) {
      Minecraft.getInstance().player.sendChatMessage("/msg " + getUsername() + " " + message);
    }
